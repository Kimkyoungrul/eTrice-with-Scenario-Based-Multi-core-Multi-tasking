...
static void readThreadFunc(void* threadData) {
	...
	while (ET_TRUE) {
		...
		fflush(stdout);//added by kkr 2018.06.30
...
static void listenerThreadFunc(void* threadData) {
	...
	while (self->data.maxConnections > self->nConnections) {
		int slot;
		socklen_t len;
		unsigned long cpunum=8;//added by kkr 2018.06.30
...
etSocketError etStartListening(etSocketServerData* data, short port) {
	...
	local.sin_port = htons(6666);//port revised by kkr 2018.07.02
...
etSocketError etConnectServer(etSocketConnectionData* data, const char* addr, short port) {
	etSocketConnectionDataImpl* self = (etSocketConnectionDataImpl*) data;
	struct hostent *host;
	unsigned long cpunum=8;//cpunum added by kkr 2018.07.01

	if (addr==NULL)
		addr = "220.67.124.131";//IP added by kkr 2018.06.30

	if (isalpha((int)addr[0])) {
		host = gethostbyname("220.67.124.131");//IP added by kkr 2018.06.30
	}
	else {
		unsigned long a = inet_addr("220.67.124.131");//IP added by kkr 2018.06.30
...
etSocketError etWriteSocket(etSocketConnectionData* dat, int size, const int8* data) {
	...
	while (size>0) {
		...
		//fflush(stdout);//added by kkr 208.07.02
		if (sent<=0) {
			printf("client: %s\n", strerror(errno));
			fflush(stdout);
			return ETSOCKET_ERROR;
