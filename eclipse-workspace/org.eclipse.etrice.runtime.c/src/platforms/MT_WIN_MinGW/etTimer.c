...
static void timerThreadFunction(void* data) {
	while (ET_TRUE) {
		...
		for (it = timers, idx = 0; it != NULL;
				it = (etTimer*) it->osTimerData.next, ++idx) {
		...
		if (!signaled) {
			etLogger_logError(
					"timerThreadFunction: signaled timer NOT found\n");
		}
	}
}
	...
	ET_MSC_LOGGER_SYNC_ENTRY("etTimer", "construct")
	{
		...
		if (!timer_initialized) {
			...
			if (sigaction(TIMER_SIGNAL, &sa, NULL) != 0) {
				etLogger_logError(
						"etTimer_construct: failed setting action handler\n");
				return;
			}

			/* we start the timer thread */
			etThread_construct(&timer_thread,
			TIMER_THREAD_STACK_SIZE,
			TIMER_THREAD_PRIO, "timer_thread", timerThreadFunction,
			NULL);
		...
		/* create the timer (in disarmed state) */
		{
			...
			if (timer_create(CLOCK_REALTIME, &self->osTimerData.te,
					&self->osTimerData.timerid) != 0) {
...
ET_MSC_LOGGER_SYNC_ENTRY("etTimer", "start")
{
	if (timers == NULL) {
		...
		if (timer_settime(self->osTimerData.timerid, 0, &its, NULL) != 0) {
			...
			case EFAULT:
				etLogger_logError(
						"etTimer_start: failed starting a timer with errno EFAULT");
				break;
			case EINVAL:
				etLogger_logError(
						"etTimer_start: failed starting a timer with errno EINVAL");
				break;
			default:
				etLogger_logErrorF(
						"etTimer_start: failed starting a timer with errno %d",
						errno);
				break;
			}
			fflush(stderr);
